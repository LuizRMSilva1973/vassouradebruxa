...
mkdir -p "$OUTDIR"
vina --receptor "${TARGETS_DIR}/${T}.pdbqt" --ligand "${LIGANDS_DIR}/${L}.pdbqt" \
  --center_x "$cx" --center_y "$cy" --center_z "$cz" \
  --size_x "$sx" --size_y "$sy" --size_z "$sz" \
  --exhaustiveness "${EXHAUSTIVENESS:-12}" --num_modes "${NUM_MODES:-9}" \
  --out "${OUTDIR}/${L}_on_${T}.pdbqt" 2>&1 | tee "${OUTDIR}/${L}_on_${T}.log"
