...
Ambiente:
  ...
  VINA_SEED         Define a semente para o Vina
...
# 9.1) filtros opcionais por regex (bash) usando TARGETS_FILTER e LIGANDS_FILTER
if [[ -n "${TARGETS_FILTER:-}" && ${#TARGETS_PDBQT[@]} -gt 0 ]]; then
  TMP=()
  for t in "${TARGETS_PDBQT[@]}"; do
    if [[ "$(basename "$t")" =~ ${TARGETS_FILTER} ]]; then TMP+=("$t"); fi
  done
  TARGETS_PDBQT=("${TMP[@]}")
fi
if [[ -n "${LIGANDS_FILTER:-}" && ${#LIGANDS_PDBQT[@]} -gt 0 ]]; then
  TMP=()
  for l in "${LIGANDS_PDBQT[@]}"; do
    if [[ "$(basename "$l")" =~ ${LIGANDS_FILTER} ]]; then TMP+=("$l"); fi
  done
  LIGANDS_PDBQT=("${TMP[@]}")
fi
...
if command -v parallel >/dev/null 2>&1; then
  ...
  for t in "${TARGETS_PDBQT[@]}"; do
    for l in "${LIGANDS_PDBQT[@]}"; do
      # Não abortar o lote em caso de falha de um par
      if ! dock_pair "${t}" "${l}"; then
        echo "[WARN] Falha no par $(basename "$l" .pdbqt) x $(basename "$t" .pdbqt); seguindo em frente." >&2
      fi
    done
  done
fi
