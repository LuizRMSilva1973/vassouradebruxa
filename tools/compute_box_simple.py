#!/usr/bin/env python3
"""
Compute Vina box (center/size) by parsing PDB or PDBQT text directly (no Biopython).
...
Usage:
  ...
  python3 tools/compute_box_simple.py --pdb targets/FKS.pdbqt --fixed-size 26.0 --out targets/FKS.box
"""
import argparse
...
def main():
    ...
    if not pdb.exists() or pdb.stat().st_size == 0:
        sys.exit(f"[ERROR] Receptor file not found or empty: {pdb}")
    residues = parse_res_list(args.res) if args.res else None
    coords = collect_coords(pdb, het=args.het, residues=residues)
    if not coords:
        # Print helpful diagnostics (first lines)
        head = ''.join(pdb.open('r', encoding='utf-8', errors='ignore').readlines()[:3]).strip()
        sys.exit("[ERROR] No ATOM/HETATM coordinates found. The file may be invalid or HTML.\n"
                 f"Path: {pdb}\nFirst lines:\n{head}")
    if args.fixed_size:
