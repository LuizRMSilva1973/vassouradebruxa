...
def main():
    ...
    prop_map = {d["ligand"]: d for d in props}

    # If ref-target provided, compute best ΔG for ref per ligand
    ref_best = {}
    if args.ref_target:
        for d in rows:
            if d.get("target") == args.ref_target:
                ...
                if dg is None:
                    continue
                if lig not in ref_best or dg < ref_best[lig]:
                    ref_best[lig] = dg
    ...
    for d in rows:
        ...
        if args.ref_target and lig in ref_best and dg is not None:
            ddg = dg - ref_best[lig]
    ...
    for r in scored:
        ...
        if lig not in best_per_lig or (sc is not None and sc > to_float(best_per_lig[lig].get("score"))):
            best_per_lig[lig] = r
    ...
    with out_rank.open("w", encoding="utf-8", newline="") as f:
        ...
        w.writeheader()
        for lig in sorted(best_per_lig.keys()):
            r = best_per_lig[lig]
